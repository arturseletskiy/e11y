...
# All options have sensible defaults. Uncomment and modify only what you need.
#
...
E11y.configure do |config|
  ...
  # Default: derived from Rails.application.class.module_parent_name
  ...
  # Default: false (enabled by Railtie when present)
  config.rails_instrumentation_enabled = true

  # Override event class for specific ASN pattern. Hash: pattern => EventClass.
  # Example: config.rails_instrumentation_custom_mappings["sql.active_record"] = MyCustomEvent
  # config.rails_instrumentation_custom_mappings = {}
  ...
  # Regex patterns to ignore in logger bridge messages. Example: /Started GET/, /Completed \d+ OK/
  ...
  # Default: true (fast feedback for web requests). Set false for background jobs to avoid failing business logic.
  ...
  # HTTP status codes to exclude from SLO calculations (e.g. 404, 401).
  ...
  # Per-job SLO config. Use add_slo_job to exclude jobs or set targets.
  ...
  # config.security_baggage_protection_allowed_keys = %w[trace_id span_id environment version service_name deployment_id request_id experiment experiment_id tenant feature_flag]
